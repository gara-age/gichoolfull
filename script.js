...
wrapperDiv.classList.add("table-wrapper"); // div.table-wrapper 생성

quizData.forEach((q, i) => {
  const table = document.createElement("table");
  table.classList.add("multi-table"); // 실제 반복할 table
...
for (let j = 0; j < q.options.length; j += 2) {
  ...
  for (let k = 0; k < 2; k++) {
    if (q.options[j + k]) {
      const index = j + k;
      choicesTable += `<td>
  <label class="choice-button" data-index="${index}" data-question="${i}">
    <input type="radio" name="q${i}" value="${index}" hidden>
    <span class="choice-symbol">${['①', '②', '③', '④'][index]}</span>
    <span class="choice-content">${q.options[j + k]}</span>
  </label>
</td>`;
    }
  }
...
document.addEventListener("click", function (e) {
  if (e.target.closest(".choice-button")) {
    ...
    // 같은 질문 보기 초기화
    document
      .querySelectorAll(`.choice-button[data-question="${questionIndex}"]`)
      .forEach(btn => {
        btn.classList.remove("selected");
        const idx = btn.dataset.index;
        btn.querySelector(".choice-symbol").textContent = symbols[idx]; // 기호만 복원
      });

    // 선택된 보기 강조 및 기호 변경
    button.classList.add("selected");
    const selectedIndex = button.dataset.index;
    button.querySelector(".choice-symbol").textContent = selectedSymbols[selectedIndex];
    ...
    const percent = (answered / quizData.length) * 100;
    currentCount.textContent = answered;
  ...
  wrapperDiv.appendChild(table); // div 안에 table 추가
});

quizContainer.appendChild(wrapperDiv); // 마지막에 한 번만 추가
...
document.getElementById("submit-btn").addEventListener("click", () => {
  ...
  const resultText = `총 ${quizData.length}문제 중 ${score}개 정답! (${(score / quizData.length) * 100}점)`;
  document.getElementById("result").textContent = resultText;
});
let timeLeft = 90 * 60; // 90분을 초 단위로
...
document.getElementById("submit-btn").addEventListener("click", () => {
  ...
  // 기존 퀴즈 화면 숨기고 결과 화면 보여주기
  document.querySelector(".exam-container").style.display = "none";
  const resultScreen = document.getElementById("result-screen");
  resultScreen.style.display = "block";
  resultScreen.style.color = "white";
  resultScreen.style.width = "600px";
  resultScreen.style.borderRadius = "10px";
  resultScreen.style.margin = "50px auto 0";

  // 텍스트 출력
  document.getElementById("score-text").textContent = `${score * 2}/100점`;
  document.getElementById("pass-fail").textContent = passed ? "합격" : "불합격";  
  document.getElementById("pass-fail-icon").textContent = passed ? '\u2B55' : '\u274C';
  document.getElementById("pass-fail-icon").style.color = passed ? "green" : "blue";
  document.getElementById("pass-fail").style.color = passed ? "green" : "red";
  document.getElementById("score-bar").value = percentage;
  

  document.getElementById("total-count").textContent = total + "문제";
  document.getElementById("correct-count").textContent = score + "문제";
  document.getElementById("wrong-count").textContent = (total - score) + "문제";

  // 오답 리스트 저장
  window.wrongQuestions = wrongList;
});

function reviewWrongAnswers() {
  // 결과 화면 숨기고 문제 화면 다시 보이기
  document.getElementById("result-screen").style.display = "none";
  document.querySelector(".exam-container").style.display = "block";

  // 모든 문제 초기화
  document.querySelectorAll(".multi-table").forEach((table, index) => {
    table.style.border = "none";
    table.style.backgroundColor = "";

    // 모든 보기 초기화
    table.querySelectorAll(".choice-button").forEach(btn => {
  ...
  // 오답 문제 표시
window.wrongQuestions.forEach(index => {
  const table = document.querySelectorAll(".multi-table")[index];
  ...
  // 사용자가 선택한 오답 보기 강조 (빨간색)
  if (selected) {
    const selectedValue = selected.value;
    selectedBtn = table.querySelector(`.choice-button[data-index="${selectedValue}"]`);
    if (selectedBtn) {
      selectedBtn.style.backgroundColor = "#ff5f5f";
      selectedBtn.classList.add("wrong", "disabled");
    }
  }

  // 정답 보기 강조 (초록색)
  correctBtn = table.querySelector(`.choice-button[data-index="${correctIndex}"]`);
  if (correctBtn) {
    correctBtn.style.backgroundColor = "#6ee7b7";
    correctBtn.classList.add("correct", "disabled");
  }
});

  // 스크롤 맨 위로
  window.scrollTo({ top: 0, behavior: "smooth" });
  // 진행상황 및 타이머 비활성화 (스타일 적용)
document.querySelector(".status").style.opacity = "0.3";
document.querySelector(".status").style.pointerEvents = "none";
clearInterval(timerInterval);
}
const timerInterval = setInterval(updateTimer, 1000); // 1초마다 실행

function openIframeModal() {
    // iframe src 설정
    document.getElementById('subscription-frame').src = './loggedin-sub-purchase.html';
